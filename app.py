...
@st.cache
def make_future(df,per,mod):
    
    futureplot= mod.make_future_dataframe(df,periods=per,n_historic_predictions=len(df))
    forecast = mod.predict(futureplot)
    return forecast
    
fore=make_future(Data2,n_days,model)
fig1=model.plot(fore)
st.plotly_chart(fig1)
st.write('Next ',n_days,' Days Predictions')
st.write(fore[['ds','yhat1']].tail(n_days))
